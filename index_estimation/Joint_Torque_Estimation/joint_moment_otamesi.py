...
filepath = rf'C:\Users\yota0\Desktop\kamiya\卒業論文\データ\result\S2_copy\70RM_1'
filepath2 = rf'C:\Users\yota0\Desktop\kamiya\卒業論文\データ\result\S2_copy\70RM_1'
filepath3 = rf'C:\Users\yota0\Desktop\otamesi\from_copy'
filepath4 = rf'C:\Users\yota0\Desktop\otamesi\from_copy'

for dir in glob.glob(rf'{filepath}\*'):
    folder = os.path.basename(dir)
    #subject = folder.replace("_length.csv", "")
    subject = "deguchi_70per_1"
    
    #length_path = rf'{dir}\length.csv'
    if not dir.endswith("length.csv"):
        pass
    else:
        ...
        #腰→膝，膝→足首の長さ取得
        length_data = pd.read_csv(rf'{filepath}\length.csv', encoding="shift jis")
        body_list = [length_data.iloc[0][4], length_data.iloc[0][5]]
        length_data2 = pd.read_csv(rf'{filepath}\length2.csv', encoding="shift jis")
        ...
        path_name = rf'{filepath}\range_correction.csv'
        ...
        # 全体重心位置データ
        df_bcom = -1*pd.read_csv(rf'{filepath2}\BodyCoM.csv')

        # 重量
        part_mass = pd.read_csv(rf'{filepath2}\Massdata.csv')

        # 部分重心
        part_com = -pd.read_csv(rf'{filepath2}\CoMdata.csv')
        ...
        rx_loca.append(df.loc[:, 'right_foot_x'])
        lx_loca.append(df.loc[:, 'left_foot_x'])
